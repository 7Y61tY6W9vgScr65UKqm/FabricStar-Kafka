...
object AdminUtils extends Logging with AdminUtilities {
  ...
  */
  ...
  /**
   * This is the signature of the original method. Every time this method gets called it calls the code of the original method twice.
   * Once for the original topic and once for the message topic.
   * @param zkUtils
   * @param topic
  ...
  def createOrUpdateTopicPartitionAssignmentPathInZK(zkUtils: ZkUtils,
    ...
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZKCode(zkUtils, topic, partitionReplicaAssignment, config, update)
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZKCode(zkUtils, topic.concat("_msg"), partitionReplicaAssignment,  config, update)
  }

  /**
   * This method contains the unmodified code of the original method.
   * @param zkUtils
   * @param topic
   * @param partitionReplicaAssignment
   * @param config
   * @param update
   */
  def createOrUpdateTopicPartitionAssignmentPathInZKCode(zkUtils: ZkUtils,
                                                     topic: String,
                                                     partitionReplicaAssignment: Map[Int, Seq[Int]],
                                                     config: Properties = new Properties,
                                                     update: Boolean = false) {
